...
kallisto index --index=uminor_unigenes_index --kmer-size=25 $GENESIS/MAPPING/UMINOR/ulmi_unigenes.fasta 

bash kallisto_ctr_v2.sh
bash kallisto_exp_v2.sh
...
for sample in ${list}
do
cp $GENESIS/MAPPING/CONTROL/${sample}_v2/abundance.tsv $GENESIS/COUNTING/CTR/${sample}_ctr.txt
done
...
for sample in ${list}
do
cp $GENESIS/MAPPING/EXP/${sample}_v2/abundance.tsv $GENESIS/COUNTING/EXP/${sample}_exp.txt
done

paste *_ctr.txt > counts_ctr.txt
paste *_exp.txt > counts_exp.txt
